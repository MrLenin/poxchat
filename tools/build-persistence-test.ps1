# Builds tools\out\persistence-test.exe (x64). Requires VS 2022 + gvsbuild deps
...
# The generated config.h defines USE_OPENSSL, so persistence.c's poxchat.h
...
cl /nologo /O1 /W3 /MD /D_CRT_SECURE_NO_WARNINGS `
  "/I$cfg" "/I$repo\src\common" "/I$openssl" `
  "/I$deps\include" "/I$deps\include\glib-2.0" "/I$deps\lib\glib-2.0\include" "/I$deps\include\gio-win32-2.0" `
  "$repo\tools\persistence-test.c" "$repo\src\common\persistence.c" `
  /Fo"$out\" /Fe"$out\persistence-test.exe" `
  /link "/LIBPATH:$deps\lib" glib-2.0.lib intl.lib
if ($LASTEXITCODE -ne 0) { throw "cl failed ($LASTEXITCODE)" }
Write-Host "built $out\persistence-test.exe"
