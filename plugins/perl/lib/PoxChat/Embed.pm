...
#   gui_entry
#     This is poxchat_plugin pointer that is used to remove the script from
...
sub unload {
	...
	if( $pkg_info ) {	
		...
		return PoxChat::EAT_ALL;
	} else {
		PoxChat::print( qq{"$file" is not loaded.\n} );
		return PoxChat::EAT_NONE;
	}
}
...
sub reload_all {
	my @dirs = PoxChat::get_info( "configdir" );
...
sub evaluate {
	...
	PoxChat::print $@ if $@; #print warnings

	local $Data::Dumper::Sortkeys = 1;
	local $Data::Dumper::Terse    = 1;

	if (@results > 1) {
		PoxChat::print Dumper \@results;
	}
	elsif (ref $results[0] || !$results[0]) {
		PoxChat::print Dumper $results[0];
	}
	else {
		PoxChat::print $results[0];
	}

	return PoxChat::EAT_POXCHAT;
};
...
sub find_external_pkg {
	...
	while( my @frame = caller( $level ) ) {
		return @frame if $frame[0] !~ /(?:^IRC$|^PoxChat)/;
		$level++;
	}
	return;
}
...
sub find_pkg {
	...
	while( my ($package, $file, $line) = caller( $level ) ) {
		return $package if $package =~ /^PoxChat::Script::/;
		$level++;
	}
