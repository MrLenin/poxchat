/*
 * SysInfo - sysinfo plugin for PoxChat
...
get_client (void)
{
	return g_strdup_printf ("PoxChat %s", poxchat_get_info(ph, "version"));
}
...
print_summary (gboolean announce)
{
	...
	poxchat_commandf (ph, "%s %s", announce ? "SAY" : "ECHO", output);
...
print_info (char *info, gboolean announce)
{
	...
	for (i = 0; hwinfos[i].name != NULL; i++)
	{
		if (!g_ascii_strcasecmp (info, hwinfos[i].name))
		{
			...
			if (str)
			{
				poxchat_commandf (ph, "%s \002%s\002: %s", announce ? "SAY" : "ECHO",
									hwinfos[i].title, str);
				g_free (str);
			}
			else
				poxchat_print (ph, _("Sysinfo: Failed to get info. Either not supported or error."));
			return;
		}
	}

	poxchat_print (ph, _("Sysinfo: No info by that name\n"));
}

static gboolean
sysinfo_get_bool_pref (const char *pref, gboolean def)
{
	int value = poxchat_pluginpref_get_int (ph, pref);
...
sysinfo_set_pref_real (const char *pref, char *value, gboolean def)
{
	if (value && value[0])
	{
		...
		poxchat_pluginpref_set_int (ph, pref, i != 0);
		poxchat_printf (ph, _("Sysinfo: %s is set to: %d\n"), pref, i != 0);
	}
	else
	{
		poxchat_printf (ph, _("Sysinfo: %s is set to: %d\n"), pref,
...
sysinfo_set_pref (char *key, char *value)
{
	if (!key || !key[0])
	{
		poxchat_print (ph, _("Sysinfo: Valid settings are: announce and hide_* for each piece of information. e.g. hide_os. Without a value it will show current (or default) setting.\n"));
		return;
	}
	...
	poxchat_print (ph, _("Sysinfo: Invalid variable name\n"));
}
...
sysinfo_cb (char *word[], char *word_eol[], void *userdata)
{
	...
	channel_type = poxchat_list_int (ph, NULL, "type");
...
poxchat_plugin_init (poxchat_plugin *plugin_handle, char **plugin_name, char **plugin_desc, char **plugin_version, char *arg)
{
	...
	*plugin_version = version;
	...
	poxchat_hook_command (ph, "SYSINFO", POXCHAT_PRI_NORM, sysinfo_cb, sysinfo_help, NULL);

	poxchat_command (ph, "MENU ADD \"Window/Send System Info\" \"SYSINFO\"");
	poxchat_toastf (ph, POXCHAT_TOAST_INFO, _("%s plugin loaded"), name);
	return 1;
}
...
{
	poxchat_command (ph, "MENU DEL \"Window/Display System Info\"");
	poxchat_toastf (ph, POXCHAT_TOAST_INFO, _("%s plugin unloaded"), name);
	return 1;
}
