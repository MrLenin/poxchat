                   ...
                   time_t redact_time)
{
	...
	/* Snapshot original, swap in the placeholder, mark redacted.  The
	 * helper suppresses duplicates so a server retransmit (chathistory
	 * replay, rejoin) can't kick a revealed entry back to REDACTED. */
	gtk_xtext_entry_apply_redaction (buf, ent, redacted_by, reason, redact_time);
}
