...
struct _poxchat_hook
{
	poxchat_plugin *pl;	/* the plugin to which it belongs */
	char *name;			/* "xdcc" */
...
typedef int (poxchat_cmd_cb) (char *word[], char *word_eol[], void *user_data);
typedef int (poxchat_serv_cb) (char *word[], char *word_eol[], void *user_data);
typedef int (poxchat_print_cb) (char *word[], void *user_data);
typedef int (poxchat_serv_attrs_cb) (char *word[], char *word_eol[], poxchat_event_attrs *attrs, void *user_data);
typedef int (poxchat_print_attrs_cb) (char *word[], poxchat_event_attrs *attrs, void *user_data);
typedef int (poxchat_fd_cb) (int fd, int flags, void *user_data);
typedef int (poxchat_timer_cb) (void *user_data);
typedef int (poxchat_init_func) (poxchat_plugin *, char **, char **, char **, char *);
typedef int (poxchat_deinit_func) (poxchat_plugin *);
...
static int
plugin_free (poxchat_plugin *pl, int do_deinit, int allow_refuse)
{
	GSList *list, *next;
	poxchat_hook *hook;
	poxchat_deinit_func *deinit_func;

	/* fake plugin added by poxchat_plugingui_add() */
	if (pl->fake)
		goto xit;
...
static poxchat_plugin *
plugin_list_add (poxchat_context *ctx, char *filename, const char *name,
					  const char *desc, const char *version, void *handle,
					  void *deinit_func, int fake, int free_strings)
{
	poxchat_plugin *pl;

	pl = g_new (poxchat_plugin, 1);
	pl->handle = handle;
...
#ifndef WIN32
static void *
poxchat_dummy (poxchat_plugin *ph)
{
	return NULL;
}
...
static int
poxchat_read_fd (poxchat_plugin *ph, GIOChannel *source, char *buf, int *len)
{
	GError *error = NULL;
				...
				void *deinit_func, char *arg, int fake)
{
	...
	if (!fake)
	{
		...
		pl->poxchat_hook_command = poxchat_hook_command;
		pl->poxchat_hook_server = poxchat_hook_server;
		pl->poxchat_hook_print = poxchat_hook_print;
		pl->poxchat_hook_timer = poxchat_hook_timer;
		pl->poxchat_hook_fd = poxchat_hook_fd;
		pl->poxchat_unhook = poxchat_unhook;
		pl->poxchat_print = poxchat_print;
		pl->poxchat_printf = poxchat_printf;
		pl->poxchat_command = poxchat_command;
		pl->poxchat_commandf = poxchat_commandf;
		pl->poxchat_nickcmp = poxchat_nickcmp;
		pl->poxchat_set_context = poxchat_set_context;
		pl->poxchat_find_context = poxchat_find_context;
		pl->poxchat_get_context = poxchat_get_context;
		pl->poxchat_get_info = poxchat_get_info;
		pl->poxchat_get_prefs = poxchat_get_prefs;
		pl->poxchat_list_get = poxchat_list_get;
		pl->poxchat_list_free = poxchat_list_free;
		pl->poxchat_list_fields = poxchat_list_fields;
		pl->poxchat_list_str = poxchat_list_str;
		pl->poxchat_list_next = poxchat_list_next;
		pl->poxchat_list_int = poxchat_list_int;
		pl->poxchat_plugingui_add = poxchat_plugingui_add;
		pl->poxchat_plugingui_remove = poxchat_plugingui_remove;
		pl->poxchat_emit_print = poxchat_emit_print;
#ifdef WIN32
		pl->poxchat_read_fd = (void *) poxchat_read_fd;
#else
		pl->poxchat_read_fd = poxchat_dummy;
#endif
		pl->poxchat_list_time = poxchat_list_time;
		pl->poxchat_gettext = poxchat_gettext;
		pl->poxchat_send_modes = poxchat_send_modes;
		pl->poxchat_strip = poxchat_strip;
		pl->poxchat_free = poxchat_free;
		pl->poxchat_pluginpref_set_str = poxchat_pluginpref_set_str;
		pl->poxchat_pluginpref_get_str = poxchat_pluginpref_get_str;
		pl->poxchat_pluginpref_set_int = poxchat_pluginpref_set_int;
		pl->poxchat_pluginpref_get_int = poxchat_pluginpref_get_int;
		pl->poxchat_pluginpref_delete = poxchat_pluginpref_delete;
		pl->poxchat_pluginpref_list = poxchat_pluginpref_list;
		pl->poxchat_hook_server_attrs = poxchat_hook_server_attrs;
		pl->poxchat_hook_print_attrs = poxchat_hook_print_attrs;
		pl->poxchat_emit_print_attrs = poxchat_emit_print_attrs;
		pl->poxchat_event_attrs_create = poxchat_event_attrs_create;
		pl->poxchat_event_attrs_free = poxchat_event_attrs_free;
		pl->poxchat_toast = poxchat_toast;
		pl->poxchat_toastf = poxchat_toastf;

		if (((poxchat_init_func *)init_func) (pl, &pl->name, &pl->desc, &pl->version, arg) == 0)
		{
...
plugin_load (session *sess, char *filename, char *arg)
{
	...
	poxchat_init_func *init_func;
	poxchat_deinit_func *deinit_func;

	if (handle == NULL)
		return (char *)g_module_error ();

	/* find the init routine poxchat_plugin_init */
	if (!g_module_symbol (handle, "poxchat_plugin_init", (gpointer *)&init_func))
	{
		g_module_close (handle);
		return _("No poxchat_plugin_init symbol; is this really a PoxChat plugin?");
	}

	/* find the plugin's deinit routine, if any */
	if (!g_module_symbol (handle, "poxchat_plugin_deinit", (gpointer *)&deinit_func))
		deinit_func = NULL;
...
plugin_get_libdir (void)
{
	...
	libdir = g_getenv ("POXCHAT_LIBDIR");
	if (libdir && *libdir)
		return libdir;
	else
		return POXCHATLIBDIR;
}
...
plugin_hook_run (session *sess, char *name, char *word[], char *word_eol[],
				 poxchat_event_attrs *attrs, int type)
{
	GSList *list, *next;
	poxchat_hook *hook;
	int ret, eat = 0;
	...
	while (1)
	{
		...
		{
		case HOOK_COMMAND:
			ret = ((poxchat_cmd_cb *)hook->callback) (word, word_eol, hook->userdata);
			break;
		case HOOK_PRINT_ATTRS:
			ret = ((poxchat_print_attrs_cb *)hook->callback) (word, attrs, hook->userdata);
			break;
		case HOOK_SERVER:
			ret = ((poxchat_serv_cb *)hook->callback) (word, word_eol, hook->userdata);
			break;
		case HOOK_SERVER_ATTRS:
			ret = ((poxchat_serv_attrs_cb *)hook->callback) (word, word_eol, attrs, hook->userdata);
			break;
		default: /*case HOOK_PRINT:*/
			ret = ((poxchat_print_cb *)hook->callback) (word, hook->userdata);
			break;
		}

		if ((ret & POXCHAT_EAT_POXCHAT) && (ret & POXCHAT_EAT_PLUGIN))
		{
			eat = 1;
			goto xit;
		}
		if (ret & POXCHAT_EAT_PLUGIN)
			goto xit;	/* stop running plugins */
		if (ret & POXCHAT_EAT_POXCHAT)
...
poxchat_event_attrs *
poxchat_event_attrs_create (poxchat_plugin *ph)
{
	return g_new0 (poxchat_event_attrs, 1);
}

void
poxchat_event_attrs_free (poxchat_plugin *ph, poxchat_event_attrs *attrs)
{
	g_free (attrs);
}
...
static int
plugin_timeout_cb (poxchat_hook *hook)
{
	int ret;
	...
	ret = ((poxchat_timer_cb *)hook->callback) (hook->userdata);
...
static void
plugin_insert_hook (poxchat_hook *new_hook)
{
	GSList *list;
	poxchat_hook *hook;
	int new_hook_type;
...
static gboolean
plugin_fd_cb (GIOChannel *source, GIOCondition condition, poxchat_hook *hook)
{
	int flags = 0, ret;
	typedef int (poxchat_fd_cb2) (int fd, int flags, void *user_data, GIOChannel *);

	if (condition & G_IO_IN)
		flags |= POXCHAT_FD_READ;
	if (condition & G_IO_OUT)
		flags |= POXCHAT_FD_WRITE;
	if (condition & G_IO_PRI)
		flags |= POXCHAT_FD_EXCEPTION;

	ret = ((poxchat_fd_cb2 *)hook->callback) (hook->pri, flags, hook->userdata, source);
...
static poxchat_hook *
plugin_add_hook (poxchat_plugin *pl, int type, int pri, const char *name,
					  const  char *help_text, void *callb, int timeout, void *userdata)
{
	poxchat_hook *hook;

	hook = g_new0 (poxchat_hook, 1);
	hook->type = type;
	hook->pri = pri;
...
void *
poxchat_unhook (poxchat_plugin *ph, poxchat_hook *hook)
{
	/* perl.c trips this */
	if (!g_slist_find (hook_list, hook) || hook->type == HOOK_DELETED)
		return NULL;
...
poxchat_hook *
poxchat_hook_command (poxchat_plugin *ph, const char *name, int pri,
						  poxchat_cmd_cb *callb, const char *help_text, void *userdata)
{
...
poxchat_hook *
poxchat_hook_server (poxchat_plugin *ph, const char *name, int pri,
						 poxchat_serv_cb *callb, void *userdata)
{
	return plugin_add_hook (ph, HOOK_SERVER, pri, name, 0, callb, 0, userdata);
}

poxchat_hook *
poxchat_hook_server_attrs (poxchat_plugin *ph, const char *name, int pri,
						   poxchat_serv_attrs_cb *callb, void *userdata)
{
...
poxchat_hook *
poxchat_hook_print (poxchat_plugin *ph, const char *name, int pri,
						poxchat_print_cb *callb, void *userdata)
{
	return plugin_add_hook (ph, HOOK_PRINT, pri, name, 0, callb, 0, userdata);
}

poxchat_hook *
poxchat_hook_print_attrs (poxchat_plugin *ph, const char *name, int pri,
						  poxchat_print_attrs_cb *callb, void *userdata)
{
...
poxchat_hook *
poxchat_hook_timer (poxchat_plugin *ph, int timeout, poxchat_timer_cb *callb,
					   void *userdata)
{
...
poxchat_hook *
poxchat_hook_fd (poxchat_plugin *ph, int fd, int flags,
					poxchat_fd_cb *callb, void *userdata)
{
	poxchat_hook *hook;
...
void
poxchat_print (poxchat_plugin *ph, const char *text)
{
	if (!is_session (ph->context))
	{
		DEBUG(PrintTextf(0, "%s\tpoxchat_print called without a valid context.\n", ph->name));
		return;
	}
...
void
poxchat_printf (poxchat_plugin *ph, const char *format, ...)
{
	va_list args;
	char *buf;
	...
	poxchat_print (ph, buf);
	g_free (buf);
}

void
poxchat_toast (poxchat_plugin *ph, const char *text, int type)
{
	...
	if (!is_session (sess))
	{
		DEBUG(PrintTextf(0, "%s\tpoxchat_toast called without a valid context.\n", ph->name));
		return;
	}
...
void
poxchat_toastf (poxchat_plugin *ph, int type, const char *format, ...)
{
	va_list args;
	char *buf;
	...
	poxchat_toast (ph, buf, type);
	g_free (buf);
}

void
poxchat_command (poxchat_plugin *ph, const char *command)
{
	char *command_utf8;

	if (!is_session (ph->context))
	{
		DEBUG(PrintTextf(0, "%s\tpoxchat_command called without a valid context.\n", ph->name));
		return;
	}
...
void
poxchat_commandf (poxchat_plugin *ph, const char *format, ...)
{
	va_list args;
	char *buf;
	...
	poxchat_command (ph, buf);
	g_free (buf);
}

int
poxchat_nickcmp (poxchat_plugin *ph, const char *s1, const char *s2)
{
...
int
poxchat_set_context (poxchat_plugin *ph, poxchat_context *context)
{
...
poxchat_context *
poxchat_find_context (poxchat_plugin *ph, const char *servname, const char *channel)
{
	return plugin_find_context (servname, channel, ph->context->server);
}

const char *
poxchat_get_info (poxchat_plugin *ph, const char *id)
{
	session *sess;
	guint32 hash;
	...
	if (!is_session (sess))
	{
		DEBUG(PrintTextf(0, "%s\tpoxchat_get_info called without a valid context.\n", ph->name));
		return NULL;
	}
...
int
poxchat_get_prefs (poxchat_plugin *ph, const char *name, const char **string, int *integer)
{
	int i = 0;
...
poxchat_list *
poxchat_list_get (poxchat_plugin *ph, const char *name)
{
	poxchat_list *list;

	list = g_new0 (poxchat_list, 1);
...
void
poxchat_list_free (poxchat_plugin *ph, poxchat_list *xlist)
{
...
int
poxchat_list_next (poxchat_plugin *ph, poxchat_list *xlist)
{
	if (xlist->next == NULL)
		return 0;
...
const char * const *
poxchat_list_fields (poxchat_plugin *ph, const char *name)
{
...
time_t
poxchat_list_time (poxchat_plugin *ph, poxchat_list *xlist, const char *name)
{
...
const char *
poxchat_list_str (poxchat_plugin *ph, poxchat_list *xlist, const char *name)
{
	guint32 hash = str_hash (name);
...
int
poxchat_list_int (poxchat_plugin *ph, poxchat_list *xlist, const char *name)
{
	guint32 hash = str_hash (name);
	gpointer data = ph->context;
...
void *
poxchat_plugingui_add (poxchat_plugin *ph, const char *filename,
...
void
poxchat_plugingui_remove (poxchat_plugin *ph, void *handle)
{
#ifdef USE_PLUGIN
...
int
poxchat_emit_print (poxchat_plugin *ph, const char *event_name, ...)
{
	va_list args;
...
char *
poxchat_gettext (poxchat_plugin *ph, const char *msgid)
{
	/* so that plugins can use PoxChat's internal gettext strings. */
	/* e.g. The EXEC plugin uses this on Windows. */
	return _(msgid);
}

void
poxchat_send_modes (poxchat_plugin *ph, const char **targets, int ntargets, int modes_per_line, char sign, char mode)
{
...
char *
poxchat_strip (poxchat_plugin *ph, const char *str, int len, int flags)
{
	return strip_color ((char *)str, len, flags);
}

void
poxchat_free (poxchat_plugin *ph, void *ptr)
{
	g_free (ptr);
}

static int
poxchat_pluginpref_set_str_real (poxchat_plugin *pl, const char *var, const char *value, int mode) /* mode: 0 = delete, 1 = save */
{
	FILE *fpIn;
	int fhOut;
	int prevSetting;
	...
	fhOut = poxchat_open_file (confname_tmp, O_TRUNC | O_WRONLY | O_CREAT, 0600, XOF_DOMODE);
	fpIn = poxchat_fopen_file (confname, "r", 0);
...
int
poxchat_pluginpref_set_str (poxchat_plugin *pl, const char *var, const char *value)
{
	return poxchat_pluginpref_set_str_real (pl, var, value, 1);
}

static int
poxchat_pluginpref_get_str_real (poxchat_plugin *pl, const char *var, char *dest, int dest_len)
{
...
int
poxchat_pluginpref_get_str (poxchat_plugin *pl, const char *var, char *dest)
{
	/* All users of this must ensure dest is >= 512... */
	return poxchat_pluginpref_get_str_real (pl, var, dest, 512);
}

int
poxchat_pluginpref_set_int (poxchat_plugin *pl, const char *var, int value)
{
	char buffer[12];

	g_snprintf (buffer, sizeof (buffer), "%d", value);
	return poxchat_pluginpref_set_str_real (pl, var, buffer, 1);
}

int
poxchat_pluginpref_get_int (poxchat_plugin *pl, const char *var)
{
	char buffer[12];

	if (poxchat_pluginpref_get_str_real (pl, var, buffer, sizeof(buffer)))
	{
		int ret = atoi (buffer);
...
int
poxchat_pluginpref_delete (poxchat_plugin *pl, const char *var)
{
	return poxchat_pluginpref_set_str_real (pl, var, 0, 0);
}

int
poxchat_pluginpref_list (poxchat_plugin *pl, char* dest)
{
	FILE *fpIn;
	char confname[64];
