/* dbus-plugin.c - poxchat plugin for remote access using D-Bus
...
#define DBUS_OBJECT_PATH "/org/poxchat"
...
static guint		context_list_find_id		(poxchat_context *context);
static poxchat_context*	context_list_find_context	(guint id);
...
{
	poxchat_list_free (ph, (poxchat_list*)data);
}
...
remote_object_finalize (GObject *obj)
{
	...
	poxchat_plugingui_remove (ph, self->handle);
...
remote_object_init (RemoteObject *obj)
{
	...
	obj->last_hook_id = 0;
	obj->last_list_id = 0;
	obj->context = poxchat_get_context (ph);
}
...
remote_object_class_init (RemoteObjectClass *klass)
{
	...
	signals[SERVER_SIGNAL] =
		g_signal_new ("server_signal",
			      ...
			      0,
			      NULL, NULL,
			      _poxchat_marshal_VOID__POINTER_POINTER_UINT_UINT,
			      G_TYPE_NONE,
		       ...
		       DBusGMethodInvocation *context)
{
	...
	remote_object->handle = poxchat_plugingui_add (ph,
...
remote_object_command (RemoteObject *obj,
		       ...
		       GError **error)
{
	if (poxchat_set_context (ph, obj->context)) {
		poxchat_command (ph, command);
	}
	return TRUE;
}
			    ...
			    GError **error)
{
	...
	context = poxchat_find_context (ph, server, channel);
			...
			GError **error)
{
	...
	*ret_info = g_strdup (poxchat_get_info (ph, id));
	return TRUE;
}
			 ...
			 GError **error)
{
	...
	*ret_type = poxchat_get_prefs (ph, name, &str, ret_int);
		...
		void *userdata)
{
	...
	info->obj->context = poxchat_get_context (ph);
			    ...
			    GError **error)
{
	...
	info->hook = poxchat_hook_command (ph,
					 name,
					 priority,
					 command_hook_cb,
					 help_text,
			   ...
			   GError **error)
{
	...
	info->return_value = return_value;
	info->id = ++obj->last_hook_id;
	info->hook = poxchat_hook_server (ph,
					name,
					priority,
					server_hook_cb,
					info);
			  ...
			  GError **error)
{
	...
	info->return_value = return_value;
	info->id = ++obj->last_hook_id;
	info->hook = poxchat_hook_print (ph,
				       name,
				       priority,
				       print_hook_cb,
				       info);
			...
			GError **error)
{
	...
	xlist = poxchat_list_get (ph, name);
	if (xlist == NULL) {
		*ret_id = 0;
		return TRUE;
	}
			 ...
			 GError **error)
{
	...
	*ret = poxchat_list_next (ph, xlist);
			...
			GError **error)
{
	...
	xlist = g_hash_table_lookup (obj->lists, &id);
	if (xlist == NULL && !poxchat_set_context (ph, obj->context)) {
		*ret_str = NULL;
		return TRUE;
	}
	...
	*ret_str = g_strdup (poxchat_list_str (ph, xlist, name));
			...
			GError **error)
{
	...
	if (g_str_equal (name, "context")) {
		poxchat_context *context;
		context = (poxchat_context*)poxchat_list_str (ph, xlist, name);
		*ret_int = context_list_find_id (context);
	} else {
		*ret_int = poxchat_list_int (ph, xlist, name);
	}
			 ...
			 GError **error)
{
	...
	*ret_time = poxchat_list_time (ph, xlist, name);
...
remote_object_list_fields (RemoteObject *obj,
			   ...
			   GError **error)
{
	*ret = g_strdupv ((char**)poxchat_list_fields (ph, name));
	if (*ret == NULL) {
		*ret = g_new0 (char*, 1);
	}
	return TRUE;
			  ...
			  GError **error)
{
	...
	*ret = poxchat_set_context (ph, obj->context);
	if (*ret) {
		*ret = poxchat_emit_print (ph, event_name, argv[0], argv[1],
							 argv[2], argv[3]);
	}
...
remote_object_nickcmp (RemoteObject *obj,
		       ...
		       GError **error)
{
	poxchat_set_context (ph, obj->context);
	*ret = poxchat_nickcmp (ph, nick1, nick2);
	return TRUE;
}
...
remote_object_strip (RemoteObject *obj,
		     ...
		     GError **error)
{
	*ret_str = poxchat_strip (ph, str, len, flag);
	return TRUE;
}
...
init_dbus (void)
{
	...
	if (connection == NULL) {
		poxchat_printf (ph, _("Couldn't connect to session bus: %s\n"),
			      error->message);
		g_error_free (error);
		return FALSE;
	}
	...
	if (!dbus_g_proxy_call (proxy, "RequestName", &error,
				...
				G_TYPE_INVALID)) {
		poxchat_printf (ph, _("Failed to acquire %s: %s\n"),
			      DBUS_SERVICE,
			      error->message);
		g_error_free (error);
...
static guint
context_list_find_id (poxchat_context *context)
{
	GList *l = NULL;
		 ...
		 void *userdata)
{
	...
	info->context = poxchat_get_context (ph);
...
int
dbus_plugin_init (poxchat_plugin *plugin_handle,
		  char **plugin_name,
		  char **plugin_desc,
		  char **plugin_version,
		  char *arg)
{
	...
	*plugin_version = PVERSION;
	...
	if (init_dbus()) {
		/*poxchat_printf (ph, _("%s loaded successfully!\n"), PNAME);*/
		...
		poxchat_hook_print (ph, "Open Context",
				  POXCHAT_PRI_NORM,
				  open_context_cb,
				  NULL);

		poxchat_hook_print (ph, "Close Context",
				  POXCHAT_PRI_NORM,
				  close_context_cb,
				  NULL);

		poxchat_hook_command (ph, "unload",
				    POXCHAT_PRI_HIGHEST,
