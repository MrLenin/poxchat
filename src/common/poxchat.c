...
static int in_poxchat_exit = FALSE;
int poxchat_is_quitting = FALSE;
/* command-line args */
...
static int
poxchat_lag_check (void)   /* this gets called every 30 seconds */
{
	lag_check ();
	return 1;
}
...
static int
poxchat_lag_check_update (void)   /* this gets called every 0.5 seconds */
{
	lagcheck_update ();
	return 1;
}
...
poxchat_reinit_timers (void)
{
	...
	{
		lag_check_update_tag = fe_timeout_add (500, poxchat_lag_check_update, NULL);
	}
	...
	if ((prefs.hex_net_ping_timeout != 0 || prefs.hex_gui_lagometer)
	    && lag_check_tag == 0)
	{
		lag_check_tag = fe_timeout_add_seconds (30, poxchat_lag_check, NULL);
	}
...
session_free (session *killsess)
{
	...
	if (!sess_list && !in_poxchat_exit)
		poxchat_exit ();						/* sess_list is empty, quit! */
...
{
#ifdef WIN32
	char poxchat_lang[13];	/* LC_ALL= plus 5 chars of hex_gui_lang and trailing \0 */

	strcpy (poxchat_lang, "LC_ALL=");

	if (0 <= prefs.hex_gui_lang && prefs.hex_gui_lang < LANGUAGES_LENGTH)
		strcat (poxchat_lang, languages[prefs.hex_gui_lang]);
	else
		strcat (poxchat_lang, "en");
